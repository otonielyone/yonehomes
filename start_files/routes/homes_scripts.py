...
def replace_and_cleanup_tables():
    ...
    try:
        ...
        # Insert all records from the temporary table into the original table
        ...
        try:
            with open(csv_path, mode='r') as data:
                ...
                for row in data_content:
                    ...
                    min_price = 20000
                    if (max_price is None or price < max_price and price > min_price) and state == 'VA':
...
def check_and_recreate_images(db, pending_dir, mls, all_imgs, max_images):
    parent_dir = 'start_files/static/homes_images/'
...
def clean_up_temp_listings(sorted_results):
    ...
    try:
        with get_db_session_pending() as db:
            temp_listings = db.query(Mls_homes_temp).filter(~Mls_homes_temp.mls.in_(results)).yield_per(100) 
            deleted_count = 0
...
async def start_task_in_loop(concurrency_limit, timeout, max_images, min_images, max_price, max_retries, delay) -> None:
    ...
    try:
        ...
        logger.info("replace_and_cleanup_tables completed")
    except Exception:   
        logger.info("replace_and_cleanup_tables failed")
