...
from start_files.models.mls.rentals import rentals_sessionLocal, Base, Mls_rentals, replace_old_db, rentals_engine
...
async def loop_task(concurrency_limit, timeout, max_images, min_images, max_price, max_retries, delay) -> None:
    ...
    Base.metadata.drop_all(rentals_engine)
    Base.metadata.create_all(rentals_engine)
...
def start_rentals(concurrency_limit, timeout, max_images, min_images, max_price, max_retries, delay):
    ...
    del_dir = f'/var/www/html/fastapi_project/start_files/static/rentals_images/'
    try:
